...
export default function MyTicketsPage() {
  ...
  const currentTickets = React.useMemo(() => 
    tickets.filter(ticket => 
      ticket.status === 'paid' && ticket.event && ticket.event.eventDate && isCurrentTicket(ticket.event.eventDate)
    ), [tickets]
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <AnimatedSection animation="fadeIn" className="mb-8 sm:mb-10 lg:mb-12">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4">
            <h2 
              className="text-2xl sm:text-3xl font-editorial-ultralight text-[#b88c41]"
              style={{ 
            ...
            {currentTickets.length > 0 && (
              <button
                ...
                className="flex items-center gap-2 bg-[#b88c41] hover:bg-[#b88c41] text-[#0A0A0A] px-4 sm:px-6 py-2 sm:py-3 rounded-full font-suisse-intl-mono text-xs sm:text-sm uppercase tracking-wide transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none w-full sm:w-auto justify-center sm:justify-start"
              >
            ...
            >
              ...
              <Ticket className="mx-auto text-[#b88c41] mb-4" size={48} />
        ...
        <AnimatedSection animation="fadeIn" className="space-y-4 sm:space-y-6">
          <h2 
            className="text-2xl sm:text-3xl font-editorial-ultralight text-[#b88c41] mb-6 sm:mb-8 text-center"
            style={{ 
              textShadow: '0 0 10px rgba(212, 175, 55, 0.3)'
            }}
          >
              ...
              {tickets.map((ticket) => (
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start mb-4 gap-3 sm:gap-0">
                    <div className="flex-1">
                      <h3 className="text-lg sm:text-xl font-suisse-intl text-[#F5F1E6] mb-2">{ticket.event?.title || 'Event'}</h3>
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-xs sm:text-sm text-[#e3dcd4]/80 font-suisse-intl-mono">
                        <div className="flex items-center gap-1">
                          ...
                          <span>{ticket.event?.eventDate && ticket.event?.eventTime ? formatDateTime(ticket.event.eventDate, ticket.event.eventTime) : 'Date TBA'}</span>
                        </div>
                        <div className="flex items-center gap-1">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                    <div>
                      <h4 className="font-suisse-intl text-[#b88c41] mb-2 text-xs sm:text-sm uppercase tracking-wider">Attendees ({ticket.quantity})</h4>
                    ...
                    <div className="text-left lg:text-right">
                      <h4 className="font-suisse-intl text-[#b88c41] mb-2 text-xs sm:text-sm uppercase tracking-wider">Payment Details</h4>
                      <p className="text-xl sm:text-2xl font-suisse-intl text-[#b88c41] mb-1">
                    ...
                    <div className="mt-4 pt-4 border-t border-[#7c4d33]/30">
                      {ticket.expiresAt && new Date() > new Date(ticket.expiresAt) ? (
                        ...
                        <div className="text-center p-4 bg-gray-900/30 border border-gray-500/30 rounded-xl">
                          ...
                          <button
                            onClick={() => router.push('/')}
                            className="bg-[#b88c41] hover:bg-[#b88c41] text-[#0A0A0A] py-2 px-4 rounded-full font-suisse-intl-mono text-xs uppercase tracking-wide transition-all duration-300"
                          >
                            Book New Ticket
                          </button>
                        </div>
                            ...
                            <button
                              ...
                              className={`flex-1 py-2 sm:py-3 px-4 sm:px-6 rounded-full font-suisse-intl-mono text-xs sm:text-sm uppercase tracking-wide transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center justify-center gap-2 ${
                                ticket.expiresAt && isExpiringSoon(ticket.expiresAt)
                                  ? 'bg-[#E67373] hover:bg-[#d45a5a] text-[#0A0A0A] animate-pulse'
                                  : 'bg-[#b88c41] hover:bg-[#b88c41] text-[#0A0A0A]'
                              }`}
                            >
                              <CreditCard size={16} />
