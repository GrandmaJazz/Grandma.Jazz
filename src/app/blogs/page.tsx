...
const PlusIcon = ({ color }: { color: string }) => (
  <div
    className="absolute bottom-2 right-2 sm:bottom-4 sm:right-4 w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 border-2 border-[#0A0A0A] rounded-full flex items-center justify-center z-10 transition-transform duration-200 group-hover:scale-75"
  ...
  >
    ...
    <div>
      ...
      <div className="px-6 pb-6 flex flex-col justify-start" style={{ height: '275px' }}>
        <h2 className="text-4xl font-bold text-[#0A0A0A]">{post.title}</h2>
      </div>
    </div>
  </Link>
);
              ...
              <div className="lg:hidden space-y-6 flex flex-col items-center">
                {blogPosts.map((post, index) => (
                  <AnimatedSection key={post._id} animation="fadeIn">
                    <SmallCard post={post} color={getCardColor(index)} />
                  </AnimatedSection>
                ))}
              </div>
                    ...
                    <AnimatedSection key={index} animation="fadeIn">
                      <div className="flex justify-center items-start w-full max-w-6xl" style={{ height: '555px', gap: '100px' }}>
                        <div style={{ alignSelf: 'flex-start' }}>
                          <SmallCard post={first} color={getCardColor(index * 2)} />
                        </div>
                        {second && (
                          <div style={{ alignSelf: 'flex-end' }}>
                            <SmallCard post={second} color={getCardColor(index * 2 + 1)} />
                          </div>
                        )}
                      </div>
                    </AnimatedSection>
                          ...
                          >
                            <div style={{ alignSelf: 'flex-end' }}>
                              {evenGroup ? (
                                ...
                                <LargeCard post={first} color={getCardColor(index * 2)} />
                              )}
                            </div>
                            {second &&
                              (evenGroup ? (
                                <LargeCard post={second} color={getCardColor(index * 2 + 1)} />
                              ) : (
                                <SmallCard post={second} color={getCardColor(index * 2 + 1)} />
                              ))}
                          </div>
