    ...
    >
      <div className="relative overflow-hidden rounded-[10px] bg-[#0A0A0A] border border-[#7c4d33]/30 transition-all duration-300 hover:shadow-lg hover:shadow-[#b88c41]/10">
        ...
        <div className="relative aspect-square overflow-hidden">
          ...
          {isOutOfStock && (
            <div className="absolute inset-0 bg-[#0A0A0A] bg-opacity-60 flex items-center justify-center z-20">
              <span className="bg-[#0A0A0A] px-4 py-2 text-[#E67373] uppercase text-sm font-suisse-intl-mono border border-[#E67373] rounded-xl">
                Sold Out
              </span>
            </div>
          )}
          ...
          {hasSecondImage && (
            <div className="absolute top-3 right-3 z-10 flex space-x-1">
              <div className={`w-2 h-2 rounded-full transition-all duration-300 ${
                !isHovered ? 'bg-[#b88c41]' : 'bg-[#b88c41]/40'
              }`} />
              <div className={`w-2 h-2 rounded-full transition-all duration-300 ${
                isHovered ? 'bg-[#b88c41]' : 'bg-[#b88c41]/40'
              }`} />
        ...
        <div className="p-4">
          ...
          <p className="font-suisse-intl-mono text-[#b88c41]">${formatPrice(price)}</p>
        </div>
      </div>
    </Link>
  );
