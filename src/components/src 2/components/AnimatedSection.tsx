...
export function AnimatedSection({
  ...
  useEffect(() => {
    ...
    // Failsafe: content must NEVER stay invisible. On the homepage the hero
    // slide transforms the whole scroll container, which can stop this
    // observer ever firing — leaving the section stranded at opacity 0.
    // If we haven't been revealed shortly after mount, reveal anyway. The
    // animation is an enhancement, not a gate on the content existing.
...
export function StaggerAnimationContainer({
  ...
  useEffect(() => {
    ...
    // Same failsafe as AnimatedSection: never leave content stranded
    // invisible if the observer fails to fire.
