...
const shell = (inner: string) => `
  <div style="background:#0A0A0A;padding:32px 0;font-family:Roboto,Arial,sans-serif;color:#e3dcd4;">
    <div style="max-width:520px;margin:0 auto;background:#141414;border:1px solid rgba(180,155,115,0.25);border-radius:20px;overflow:hidden;">
      ...
      <div style="padding:20px 32px;border-top:1px solid rgba(180,155,115,0.2);font-size:12px;color:#8a847c;">
        Grandma Jazz · Kamala, Phuket · <a style="color:#B49B73;" href="https://grandmajazz.com">grandmajazz.com</a>
      </div>
    </div>
  </div>`;
...
export function familyWelcomeHtml(name: string, title: string): string {
  return shell(`
    ...
    <p style="margin-top:20px;">
      <a href="https://grandmajazz.com/family" style="display:inline-block;background:#B49B73;color:#0A0A0A;text-decoration:none;padding:12px 22px;border-radius:999px;font-size:13px;text-transform:uppercase;letter-spacing:1px;">See the wall</a>
    </p>`);
}
...
export function ticketEmailHtml(opts: {
  ...
  return shell(`
    ...
    <p><strong style="color:#B49B73;">${escapeHtml(opts.eventTitle)}</strong><br/>${escapeHtml(opts.dateLabel)}</p>
